...
async function createJob(title, description, start_date, duration, creator_id) {
  ...
  try {
    const newId = await createJob.run(title, description, start_date, duration, creator_id).lastInsertRowid;
...
async function updateJob(id, title, description, start_date, duration, creator_id) {
  const updateJob = db.prepare("UPDATE jobs SET title = ?, description = ?, start_date = ?, duration = =, creator_id = ? WHERE id = ?")
  try {
    const info = await updateJob.run(title, description, start_date, duration, creator_id, id);
    return info.changes;
  } catch (error) {
    return {error: error.code}
  }
